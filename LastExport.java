...
public class LastExport {
	...
	public static void lastExport(Record [] record)  throws IOException { //declare lastExport method

		final FileReader inputFile = new FileReader("C:/Users/Public/Documents/Collection_Export.txt"); // read the Collection_Export.txt file
		...
		inputBuffer.close(); 									// end the reading of Collection_Export.txt file
		
		viewExport(record);										// call viewExport method and pass the record array to it.
	}
	
	public static void viewExport(Record [] record)  throws IOException { // declare viewExport method
				
		for(int i=0; i<record.length; i++) { 															//declare loop to iterate for the length of the record arrays
	...
	public static void subMenu (Record [] record)  throws IOException  {
		...
		if (sub_menu != null && !sub_menu.equals("") && sub_menu.matches("((-|\\+)?[0-9]+(\\.[0-9]+)?)+")) { // chech the valid input with regular expressions to check whether "sub_menu" string contain numeric value or not. 
			...
			if (sub_menu.equals("1")) { // declare if statement to catch the input when the user inputs 1 (one)
				lastExport(record); 	// call back the lastExport method and pass the record array to it
			}
			if (sub_menu.equals("2")) { 			 // declare if statement to catch the input when the user inputs 2 (two)
				ExportRecords.exportRecords(record); // call back the exportRecords method from ExportRecords class and pass the record array to it
			}
